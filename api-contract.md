...
### `CashStatus`
```ts
type CashStatus = "PAYER_CLAIMED" | "PAYEE_CONFIRMED" | "PAYEE_DISPUTED" | "ADMIN_VERIFIED";
```
...
```ts
{
  user: { id: string; email: string; name: string; role: UserRole; profileStatus: UserProfileStatus; image: string | null };
  accessToken: string;
...
Request:
```ts
{ oldPassword: string; newPassword: string } // newPassword min 8 chars
```
...
Request:
```ts
{ token: string } // raw token from email URL
```

Response `data`:
```ts
{
  user: { id: string; email: string; name: string; role: UserRole; profileStatus: UserProfileStatus; emailVerified: boolean };
  accessToken: string;
  refreshToken: string; // also set as httpOnly cookie
}
```
...
Request:
```ts
{ image: string } // valid URL, max 255 chars
```

Response `data`:
```ts
{ id: string; email: string; name: string; image: string; updatedAt: string }
```
...
```ts
{ id: string; image: string | null; name: string; bio: string | null }
```
...
Request:
```ts
{ rejectionReason: string } // min 10, max 500 chars
```
...
Request:
```ts
{ withdrawalReason: string } // min 10, max 500 chars
```
...
```ts
{
  ...
  commissionAmount: string;
  commissionDeducted: boolean;
  task: { title: string; category: TaskCategory; location: string; status: TaskStatus };
  payer: { id: string; name: string; email: string };
  payee: { id: string; name: string; email: string };
}
```
